...
function ascendingSort(tds,tr) {
    ...
    for (var i = 0; i < len - 1; i++) {
        ...
        for (var j = 0; j < len - 1 - i; j++) {
        	...
        	time[j] = new Date(tr[j].children[7].innerText);
        	time[j+1] = new Date(tr[j+1].children[7].innerText);
        	
        	console.log(tr[j].children[7].innerText.replace(reg,"") );
        	console.log( time[j].getTime() );
	   ...
	   // var reg = new RegExp("/","g");//正则
	    ...
	    for (var i = 0; i < len - 1; i++) {    	
	        for (var j = 0; j < len - 1 - i; j++) {
	        	...
	        	//console.log( time[j].getTime() );//返回毫秒数
	            if (time[j].getTime() < time[j+1].getTime() ) {        // 相邻元素两两对比        	
	                var temp = tr[j];        // 元素交换             
	                tr[j] = tr[j+1];
	                tr[j+1] = temp;
